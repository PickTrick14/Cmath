...
START_TEST(test_1) {
  double x = 0;
  ck_assert_double_eq_tol(my_sin(x), sin(x), 1e-6);
}
END_TEST
...
START_TEST(test_16) {
  double x = -MY_PI / 3;
  ck_assert_double_eq_tol(my_sin(x), -sin(MY_PI / 3), 1e-6);
}
END_TEST

Suite *my_sin_tests(void) {
  Suite *suite = suite_create("my_sin_test:");
  TCase *tcase_core = tcase_create("my_sin");
